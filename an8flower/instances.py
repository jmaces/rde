...
def load_train_generator(path=DATAPATH, class_mode=None):
    ...
    generator_module = importlib.util.module_from_spec(generator_spec)
    generator_spec.loader.exec_module(generator_module)
    return generator_module.load_train_data(class_mode=class_mode)


def load_statistics(path=STATISTICSPATH, mode="diag", rank=100):
...
def compute_statistics(path=STATISTICSPATH, mode="diag", rank=30):
    ...
    generator, _ = load_train_generator()
