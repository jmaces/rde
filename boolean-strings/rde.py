...
def get_distortion(x, target, mean, covariance, model, mode="diag"):
    ...
    if mode == "diag":
        loss = (
            1
            / 2
            * (K.mean(K.square(out_mean - target)) + K.mean(out_covariance))
        )
    elif mode == "half":
        out_covariance = K.batch_dot(
            out_covariance, out_covariance, axes=(1, 1)
        )
        loss = (
            1
            / 2
...
def adf_rde_lagrangian(x, pred, s0, mean, covariance, model, mu, mode="diag"):
    f, df = get_distortion(x, pred, mean, covariance, model, mode)
